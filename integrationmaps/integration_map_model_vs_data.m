function S = integration_map_model_vs_data(X,Y,Ymodel, dx, sigma, bnd, nBootstrap)
...
S_model = IntegrationMap(X, Ymodel, bnd, dx, sigma);
S.IntegrationMapModel = S_model;

% APPLY INTEGRATION MAP FOR DATA
S_data = IntegrationMap(X, Y, bnd, dx, sigma);
...
if nBootstrap>0
    ...
    for b=1:nBootstrap % for each bootstrap
        ...
        Sbootstrap_Model = IntegrationMap(X(rd,:), Ymodel(rd), bnd, dx);
        Sbootstrap_Data = IntegrationMap(X(rd,:), Y(rd), bnd,dx);
        IntegrationMapDataBs = Sbootstrap_Data.IntegrationMap;
        IntegrationMapBs= Sbootstrap_Model.IntegrationMap;
        nDatapointsBtstrp = Sbootstrap_Model.nDatapoints;
